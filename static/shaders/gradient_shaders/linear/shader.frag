...
void main() {
  ...
  if (originalTexColor.r >= targetColor.r && originalTexColor.r <= targetColor.r + tolerance &&
    ...
    float position = gl_FragCoord.x * cos(radianAngle) + gl_FragCoord.y * sin(radianAngle);
    float index = mod(position + float(frameCount) * gradientDirection * speed, gradientTextureLength);
    index = floor(index);
